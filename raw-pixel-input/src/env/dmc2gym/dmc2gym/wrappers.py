...
class DMCWrapper(core.Env):
    def __init__(
        ...
        if from_pixels:
            if self._depth:
                ...
                num_channels_obs = 3
            shape = [num_channels_obs, height, width] if channels_first else [height, width, num_channels_obs]
    ...
    def _get_obs(self, time_step):
        if self._from_pixels:
            ...
            if self._depth:
                obs = np.expand_dims(obs.copy(), axis = 2)
    ...
    def render(self, mode='rgb_array', height=None, width=None, camera_id=0, depth = False):
        ...
        return self._env.physics.render(
            height=height, width=width, camera_id=camera_id, depth = depth
        )
