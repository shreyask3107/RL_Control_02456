...
def main(args):
	...
	work_dir_name = args.domain_name+'_'+args.task_name
	if args.pretrain:
		work_dir_name = work_dir_name + '_pretrain_' + args.pretrain_task_name
	if args.depth:
		work_dir_name = work_dir_name + '_depth'




	work_dir = os.path.join(args.log_dir, work_dir_name, args.algorithm, str(args.seed))
	...
	if args.depth:
		cropped_obs_shape = (args.frame_stack, args.image_crop_size, args.image_crop_size)
	else:
	...
	if args.pretrain:
		agent = torch.load(os.path.join(args.pretrain_dir, str(args.pretrain_steps)+'.pt'))
