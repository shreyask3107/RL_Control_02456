...
class SACAgent(Agent):
    ...
    def __init__(self, obs_dim, action_dim, action_range, work_dir, device, critic_cfg,
                 ...
                 batch_size, learnable_temperature, pretrain, pretrain_dir, pretrain_steps):
        super().__init__()
        ...
        if pretrain==True:
            self.actor = torch.load(os.path.join(str(pretrain_dir), 'model', str(pretrain_steps) + '_actor.pt'))
            self.critic = torch.load(os.path.join(str(pretrain_dir), 'model', str(pretrain_steps) + '_critic.pt'))
